...
module.exports = async (deployer, network, addresses) => {
  ...
  if (DEPLOY_ACCESSORIES) {
    await deployer.deploy(CreatureAccessory, proxyRegistryAddress, {
      gas: 5000000,
    });
    const accessories = await CreatureAccessory.deployed();
    await setupCreatureAccessories.setupAccessory(accessories, addresses[0]);
  }
  ...
  if (DEPLOY_ACCESSORIES_SALE) {
    ...
    await deployer.deploy(CreatureAccessoryLootBox, proxyRegistryAddress, {
      gas: 6721975,
    });
    ...
    await accessories.transferOwnership(CreatureAccessoryFactory.address);
